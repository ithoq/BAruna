...
class AppHelper {
    ...
    public static function upload_image($request, $imagename,$company_code,$isResize=true) {
        ...
        try{
            ...
            $uploader = $image->move('uploads/img', 'v-'.$upload_path.$filename.'.'.$image->getClientOriginalExtension());
            ...
            $obj['image']='/uploads/img/v-'.$upload_path.$filename.'.'.$image->getClientOriginalExtension();
            $obj['image_thumb']='/uploads/img/v-'.$upload_path.$filename.'.'.$image->getClientOriginalExtension();
     ...
     public static function upload_image_crop($image,$company_id) {
        ...
        try{
            ...
            $now = Carbon::now();
            $path = 'uploads/'.$company_id.'/'.$now->year.'/'.$now->month.'/';
            $filename = Uuid::generate();
            // $uploader = Storage::disk('local')->getDriver()->put($path.$filename, $image, [ 'visibility' => 'public', 'CacheControl' => 'max_age=2592000']);
    ...
    public static function deleteimage($image_public_id = '') {
        ...
        try{
            if (count($image_public_id) > 0) {
                unlink(public_path($image_public_id));
            }
            $obj['error']=false;
            $obj['test'] = $image_public_id;
            return $obj;
        }
        catch (\Exception $e) {
            $data['error']=true;
