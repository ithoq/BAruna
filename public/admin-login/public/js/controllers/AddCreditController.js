				...
				function($scope, $state, $stateParams, propertiesConstant, TokenStorage, masterDataServices,toaster , FileUploader) {
					...
					$scope.list_status=[
					 ...
					 var uploader = $scope.uploader = new FileUploader({
				          url: propertiesConstant.API_URL+'upload_image?company_id='+$scope.company_id
				    });
					...
					if (type === 'list') {
						...
						if ($stateParams.master==null){
							...
							$scope.list_category=masterDataServices.query({module:'add-credit_category',company_id:$scope.company_id});
							$scope.list_tags=[];
							$scope.master = $stateParams.master[0];
