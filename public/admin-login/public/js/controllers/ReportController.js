		...
		[ '$scope', '$state', '$stateParams', 'propertiesConstant', 'TokenStorage', 'masterDataServices','toaster', 'FileUploader' ,
				function($scope, $state, $stateParams, propertiesConstant, TokenStorage, masterDataServices,toaster , FileUploader) {
					...
					var uploader = $scope.uploader = new FileUploader({
				          url: propertiesConstant.API_URL+'upload_pdf?company_id='+$scope.company_id
				    });
					...
					if (type === 'list') {
						...
						$scope.thumb = function(master) {
							$state.go('app.' + module + '-thumb', {
								master : master
							});
						};
						...
						if ($stateParams.master==null){
							...
							$scope.list_category=masterDataServices.query({module:'report_category',company_id:$scope.company_id});
							$scope.list_tags=[];
							$scope.master = $stateParams.master[0];


							$scope.save_tag=function(data){
								data.post_id=$scope.master.id;
								data.company_id=$scope.company_id;
								masterDataServices.save('post_tags', data).success(function(response) {
									busy = false;
									if (response.error==true){
						                toaster.pop('error','Information',response.message);
						              }else{
							...
							$scope.remove_tags=function(master,index){
								masterDataServices.remove('post_tags', master).success(function() {
						...
						var uploader = $scope.uploader = new FileUploader({
					          url: propertiesConstant.API_URL+'report_upload_image?company_id='+$scope.company_id
					    });
