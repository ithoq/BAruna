				...
				function($scope, $state, $stateParams, propertiesConstant, TokenStorage, masterDataServices,toaster , FileUploader) {
					...
					$scope.list_status=[
					 ...
					 var uploader = $scope.uploader = new FileUploader({
				          url: propertiesConstant.API_URL+'change_featured_image?company_id='+$scope.company_id
				    });
					...
					if (type === 'list') {
						...
						if ($stateParams.master==null){
							...
							$scope.list_category=masterDataServices.query({module:'post_category',company_id:$scope.company_id});
							$scope.list_tags=[];
							if ($stateParams.master[0].id){
								$scope.list_tags=masterDataServices.query({module:'post_tags',company_id:$scope.company_id,post_id:$stateParams.master[0].id});
							}
							$scope.master = $stateParams.master[0];
