				...
				function($scope, $state, $stateParams, propertiesConstant, TokenStorage, masterDataServices,toaster , FileUploader) {
					...
					masterDataServices.company({
					 ...
					 var uploader = $scope.uploader = new FileUploader({
				          url: propertiesConstant.API_URL+'product_gallery?company_id='+$scope.company_id
				    });
				    ...
				    uploader.filters.push({
				     ...
				     uploader.onBeforeUploadItem = function(item) {
					  	   item.headers = {
							    ...
							    item.formData.push({product_id: $scope.master.id});
				    };
					...
					if (type === 'list') {
						...
						if ($stateParams.master==null){
							...
							$scope.list_facilities=masterDataServices.query({module:'product_facilities',company_id:$scope.company_id,product_id:$stateParams.master[0].id});
							$scope.list_tags=[];
							if ($stateParams.master[0].id){
								$scope.list_tags=masterDataServices.query({module:'product_tags',company_id:$scope.company_id,product_id:$stateParams.master[0].id});
							}
							$scope.master = $stateParams.master[0];

							if ($stateParams.master[0].id == null){
							}else{
								$scope.list_images=masterDataServices.query({module:'product_gallery',product_id:$stateParams.master[0].id});
							}

							$scope.setMainImages=function(id){
								var product_images = {id:$stateParams.master[0].id,image_id:id};
								masterDataServices.save('update_main_image', product_images).success(function(response) {
									busy = false;
							...
							$scope.save_tag=function(data){
								data.product_id=$scope.master.id;
								data.company_id=$scope.company_id;
								masterDataServices.save('product_tags', data).success(function(response) {
									busy = false;
									if (response.error==true){
						                toaster.pop('error','Information',response.message);
						              }else{
						...
						$scope.remove_gallery=function(master,index){
						    	masterDataServices.remove('product_gallery', master).success(function() {
						...
						$scope.remove_tags=function(master,index){
						    	masterDataServices.remove('product_tags', master).success(function() {
